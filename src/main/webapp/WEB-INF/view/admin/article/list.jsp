...
<ul class="pagination">
    ...
    <c:forEach begin="${pageInfo.pageNum-2 > 1 ? info.pageNum-2:1}" end="${pageInfo.pageNum+2 > info.pages ? info.pages:info.pageNum+2}" varStatus="index">    		
    	<c:if test="${pageInfo.pageNum!=index.index}"><li><a href="javascript:goPage(${index.index})">${index.index}</a></li></c:if>
    	<c:if test="${pageInfo.pageNum==index.index}"><li><a href="javascript:void"><strong> ${index.index} </strong> </a></li></c:if>
    </c:forEach>
...
<script type="text/javascript">
	...
	*/
	function apply(status) {
		...
		$.post("/admin/applyArticle",{id:globalArticleId,status:status},function(data){
			if(data.result==1){
				alert("审核操作成功");
